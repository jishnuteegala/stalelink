...
OOXML fixes raw-copy every untouched ZIP entry and byte-splice only matching URLs in XML relationship/document parts. PDF annotation changes are append-only incremental updates. PDF bare-text URLs require manual editing, and encrypted or signed PDFs are refused so stalelink never invalidates encryption or signatures. `--fix-exclude pdf` excludes all PDF fixes. Exit code 0 means a dry run completed or all requested fixes completed, 1 means a fix was refused or verification failed, 2 means invalid arguments/configuration, and 3 means an environment or IO setup failure.
